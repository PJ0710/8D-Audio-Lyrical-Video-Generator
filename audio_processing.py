...
def apply_effects(file_name):
    mono_wav, sampling_rate = librosa.load(file_name, duration=270)
    stereo_wav, sampling_rate = librosa.load(
        file_name, mono=False, duration=270)
    tempo, beat_frames = librosa.beat.beat_track(
        y=stereo_wav[0], sr=sampling_rate)
    ...
    while i < length - 4*end_of_beat:
        ...
        if left_up:
            ...
            stereo_wav[1, i:i+end_of_beat] = mono_wav[i:i +
            ...
            stereo_wav[0, i:i + end_of_beat] = mono_wav[i:i +
                                                        end_of_beat]*down_value
    ...
    wav1 = stereo_wav
    wavfile.write(f'./output/{file_name.split(".")[0]}-out.wav', sampling_rate, wav1.T)
    tfm = sox.Transformer()
    tfm.treble(gain_db=5, slope=0.3)
    tfm.bass(gain_db=5, slope=0.3)
    tfm.build(f"./output/{file_name.split('.')[0]}-out.wav",
              f"./output/{file_name.split('.')[0]}-effect.wav")
    return f"./output/{file_name.split('.')[0]}-effect.wav"
